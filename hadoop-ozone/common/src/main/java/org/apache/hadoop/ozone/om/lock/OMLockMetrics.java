...
public final class OMLockMetrics implements MetricsSource {
  ...
  private OMLockMetrics() {
    ...
    readLockWaitingTimeMsStat = OzoneMetricsSystem.registerNewMutableStat(
            registry,
            "ReadLockWaitingTime",
            "Time (in milliseconds) spent waiting for acquiring the read lock",
            "Ops", "Time", true);

    readLockHeldTimeMsStat = OzoneMetricsSystem.registerNewMutableStat(
            registry,
            "ReadLockHeldTime",
            "Time (in milliseconds) spent holding the read lock",
            "Ops", "Time", true);

    writeLockWaitingTimeMsStat = OzoneMetricsSystem.registerNewMutableStat(
            registry,
            "WriteLockWaitingTime",
            "Time (in milliseconds) spent waiting for acquiring the write lock",
            "Ops", "Time", true);

    writeLockHeldTimeMsStat = OzoneMetricsSystem.registerNewMutableStat(
            registry,
            "WriteLockHeldTime",
