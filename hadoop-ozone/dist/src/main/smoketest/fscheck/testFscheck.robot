...
Run FSCheck Damaged Keys
    [Arguments]    ${num_keys}
    ${result}=    Execute    ozone sh fscheck --verbosity-level KEY --output-format=PLAIN_TEXT
...
Run FSCheck With Delete
    ...
    Execute And Ignore Error    ozone sh fscheck --delete --verbosity-level KEY --output-format=PLAIN_TEXT
...
Run FSCheck With Output
    ...
    Execute And Ignore Error    ozone sh fscheck --healthy-keys --output-format=PLAIN_TEXT --output=/tmp/file.txt
...
Run FSCheck With Verbose Key
    ...
    ${result}=    Execute    ozone sh fscheck --healthy-keys --verbosity-level=KEY --output-format=PLAIN_TEXT
...
Run FSCheck With Verbose Chunk
    ...
    ${result}=    Execute    ozone sh fscheck --healthy-keys --verbosity-level=CHUNK --output-format=PLAIN_TEXT
    ${keys}=    Evaluate    ${num_keys} + 1
    FOR    ${i}    IN RANGE    1    ${keys}
        Should Contain    ${result}    ${TEST_VOLUME}/${TEST_BUCKET}/key${i}
    END
...
Run FSCheck With Verbose Block
    ...
    ${result}=    Execute    ozone sh fscheck --healthy-keys --verbosity-level=BLOCK --output-format=PLAIN_TEXT
    Should Contain    ${result}    Block commit sequence id:

Run FSCheck With Verbose Container
    Log    Running fscheck with verbose option for containers
    ${result}=    Execute    ozone sh fscheck --healthy-keys --verbosity-level=CONTAINER --output-format=PLAIN_TEXT
    Should Contain    ${result}    Container

Run FSCheck With Delete Option
    Log    Running fscheck with delete option
    ${check}=    Execute    ozone sh fscheck --delete --verbosity-level=KEY --output-format=PLAIN_TEXT
