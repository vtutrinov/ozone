...
*** Variables ***
${KEYCLOAK_URL}     http://keycloak:8080/realms/ozone/protocol/openid-connect/token
...
Keycloak Is Accessible
    ...
    ${output} =    Execute    curl -s -o /dev/null -w "%{http_code}" http://keycloak:8080/realms/ozone
    Should Be Equal As Strings    ${output}    200
...
Kerberos Is Configured
    ...
    ${value} =    Execute    ozone getconf confKey hadoop.security.authentication
...
Create Volume In Secure Cluster Without KDC
    ...
    ${output} =    Execute    ozone sh volume create /oauth-vol1
...
Create Bucket In Secure Cluster Without KDC
    ...
    ${output} =    Execute    ozone sh bucket create /oauth-vol1/oauth-bucket1
    Should Not Contain    ${output}    PERMISSION_DENIED
...
Put And Get Key In Secure Cluster Without KDC
    ...
    ${output} =    Execute    ozone sh key put /oauth-vol1/oauth-bucket1/test-key /tmp/oauth-test-input.txt
    Should Not Contain    ${output}    PERMISSION_DENIED
    Execute    ozone sh key get /oauth-vol1/oauth-bucket1/test-key /tmp/oauth-test-output.txt
...
Clean Up
    ...
    Execute And Ignore Error    ozone sh key delete /oauth-vol1/oauth-bucket1/test-key
    Execute And Ignore Error    ozone sh bucket delete /oauth-vol1/oauth-bucket1
    Execute And Ignore Error    ozone sh volume delete /oauth-vol1
