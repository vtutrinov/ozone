...
#
# SCM Kerberos principal migration: per-pod _HOST  →  stable scm/<service>@REALM.
#
# Sibling of migrate-to-stable-principal.sh (which handles OM). Same shape,
# same phase semantics — only the keys, defaults, and pod prefix differ.
#
# Use this when admin tools (ozone admin scm roles / safemode / datanode list)
# run from outside the cluster and hit "Server has invalid Kerberos principal:
# scm/<pod-fqdn>@…, expecting: scm/<lb-ip>@…" — the analogue of the OM case.
#
# What the migration does:
#   - Replaces every SCM pod's keytab with one that carries BOTH the old
#     per-pod principal AND the new stable principal (phase 3).
#   - Switches the SCM config to announce the stable principal so clients
...
#     scm-0-0.keytab, scm-1-0.keytab, scm-2-0.keytab).
#   - A slim keytab containing ONLY the stable principal, at
#     $KEYTAB_DIR/scm-scmservice-only.keytab, for the cleanup phase.
#
# Usage:
#   ./migrate-scm-to-stable-principal.sh preflight
#   ./migrate-scm-to-stable-principal.sh roll
#   ./migrate-scm-to-stable-principal.sh config
#   # → roll out the new principal to all client hosts here …
#   ./migrate-scm-to-stable-principal.sh cleanup
#
#   DRY_RUN=true ./migrate-scm-to-stable-principal.sh roll     # print, don't apply
#
...
#   SERVICE_ID         ozone SCM service ID      (default: scmservice)
#   SCM_PODS           space-separated pod list  (default: scm-0-0 scm-1-0 scm-2-0)
#   KEYTAB_DIR         where merged keytabs sit  (default: /tmp/scm-keytab-migration)
#   SECRET_NAME        Secret name               (default: scm-keytabs)
#   CONFIGMAP_NAME     ConfigMap name            (default: config)
#   KEYTAB_PATH        path inside the pod       (default: /etc/security/keytabs/scm.keytab)
...
SERVICE_ID="${SERVICE_ID:-scmservice}"
NEW_PRINC="scm/${SERVICE_ID}@${REALM}"
read -r -a SCM_PODS <<< "${SCM_PODS:-scm-0-0 scm-1-0 scm-2-0}"
KEYTAB_DIR="${KEYTAB_DIR:-/tmp/scm-keytab-migration}"
SECRET_NAME="${SECRET_NAME:-scm-keytabs}"
CONFIGMAP_NAME="${CONFIGMAP_NAME:-config}"
KEYTAB_PATH="${KEYTAB_PATH:-/etc/security/keytabs/scm.keytab}"
BACKUP_DIR="${BACKUP_DIR:-$KEYTAB_DIR/backup-$(date +%Y%m%d-%H%M%S)}"
DRY_RUN="${DRY_RUN:-false}"
CONFIG_KEY="OZONE-SITE.XML_hdds.scm.kerberos.principal"
...
# Detect the current SCM leader. Empty if not detectable (single-SCM or auth issue).
detect_leader() {
  local out=""
  for p in "${SCM_PODS[@]}"; do
    out=$(kubectl -n "$NAMESPACE" exec "$p" -- \
      ozone admin scm roles -id="$SERVICE_ID" 2>/dev/null \
...
phase_0_preflight() {
  ...
  for pod in "${SCM_PODS[@]}"; do
    if ! kubectl -n "$NAMESPACE" get pod "$pod" >/dev/null 2>&1; then
      err "Pod $pod not found in namespace $NAMESPACE"
      exit 1
    fi
  done
  ok "All ${#SCM_PODS[@]} pods present in $NAMESPACE"
  ...
  for pod in "${SCM_PODS[@]}"; do
    ...
    if [[ ! -f "$kt" ]]; then
      ...
      err "  kadmin -q 'ktadd -norandkey -k $kt scm/${pod}.scm.${NAMESPACE}.svc.cluster.local@$REALM $NEW_PRINC'"
      exit 1
    fi
    if command -v klist >/dev/null 2>&1 ; then
      ...
      local pod_fqdn="${pod}.scm.${NAMESPACE}.svc.cluster.local"
      if ! klist -k "$kt" 2>/dev/null | grep -q "scm/$pod_fqdn" ; then
        warn "Keytab $kt does not contain the per-pod principal scm/$pod_fqdn@$REALM."
  ...
  if [[ -n "$leader" ]]; then
    ok "Current SCM leader: $leader (followers will be rolled first)"
  else
    warn "Could not detect SCM leader; pods will be rolled in configured order"
  fi
}
...
phase_3_roll_keytabs() {
  ...
  cat <<EOF
After this phase each SCM pod will:
...
EOF
  ...
  for pod in $(ordered_pods "$leader") ; do
    local pod_fqdn="${pod}.scm.${NAMESPACE}.svc.cluster.local"
    local expected="scm/$pod_fqdn@$REALM"
    roll_pods "$expected" "$pod"
  done

  ok "Phase 3 complete — pods carry both keys, still announce the old principal."
  log "Next: run '$0 config' once you're ready to switch the SCM config."
}

# ============================================================================
# Phase 4 — Switch SCM config to announce scm/<service>
# ============================================================================
phase_4_switch_config() {
  phase_header "Phase 4 — Switch SCM config: announce $NEW_PRINC"
  cat <<EOF
After this phase each SCM pod will:
  - log in as $NEW_PRINC
...
Clients dialing the LB VIP with hdds.scm.kerberos.principal=$NEW_PRINC
...
Next steps before phase 7:
  ...
  2. Roll the same hdds.scm.kerberos.principal=$NEW_PRINC into every
     client host's ozone-site.xml. Drop any prior hdds.scm.kerberos.principal.pattern=*
...
phase_7_cleanup() {
  ...
  warn "PREREQUISITE: every client must already be using hdds.scm.kerberos.principal=$NEW_PRINC."
  warn "After this phase, SCM keytabs will no longer carry the per-pod principals."
  warn "Any client still expecting the old SPN will break."

  local slim="$KEYTAB_DIR/scm-scmservice-only.keytab"
  if [[ ! -f "$slim" ]]; then
  ...
  if command -v klist >/dev/null 2>&1 ; then
    ...
    if klist -k "$slim" 2>/dev/null | grep -q 'scm/.*\.svc\.cluster\.local' ; then
  ...
  ok "Phase 7 complete — every SCM pod now holds only $NEW_PRINC."
  cat <<EOF
...
EOF
  for pod in "${SCM_PODS[@]}"; do
    local fqdn="${pod}.scm.${NAMESPACE}.svc.cluster.local"
    printf '  > delprinc scm/%s@%s\n' "$fqdn" "$REALM"
  done
  echo
}
...
Phases:
  ...
  roll       push merged keytabs to SCM pods + rolling restart  (phase 3)
  config     switch SCM config to announce $NEW_PRINC + rolling restart  (phase 4)
...
Drop these files in \$KEYTAB_DIR before running:
  ...
  scm-scmservice-only.keytab                        (cleanup phase only)
EOF
}
...
main() {
  case "${1:-}" in
    ...
    all)
      ...
      warn "until every client has switched to hdds.scm.kerberos.principal=$NEW_PRINC."
